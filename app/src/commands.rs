...
pub fn palette() -> Vec<Command> {
    vec![
        ...
        command("Export as a Postman collection", ExportCollection),
        ...
        command("Manage certificates", OpenCertificates),
        command("Choose a client certificate", ChooseClientCert),
        command("Choose a root CA certificate", ChooseRootCa),
...
const EXCLUDED: &[(&str, &str)] = &[
    ...
    ("zuno::CertsNext", "only valid inside the certificates panel"),
    ("zuno::CertsPrev", "only valid inside the certificates panel"),
