...
cargo test --workspace                   # 1068 tests, ~50s
...
Text **wraps** by default, and a fixed-height row then slices the second line | `Style::default()` is `WhiteSpace::Normal`, so `overflow_hidden` + `min_w(0)` is not enough for a one-line label: the text reflows onto line two and a `uniform_list` row, which must be a fixed height, cuts it in half. `whitespace_nowrap()` is the fix and is the reliable half of `truncate()` — unlike its ellipsis, which depends on a cached measurement, this is a plain flag the shaper reads. Worth knowing how it presents: it looks like a *rendering fault*, not like a name that is too long, so it gets misdiagnosed as clipping. It was, here, by me — asserted rather than checked. |
...
An element positioned from another element's painted bounds is **absent for one frame** | `TextInput::last_bounds` is written during `paint`, so on the frame a row is first drawn the parent's `render` has already asked and got `None`. Measured: the header-name dropdown was absent on frame one and present on frame two. Harmless for placement — a cell does not move while you type in it — but it means anything *derived* from those bounds is unobservable for a frame, so keep the **contents** of such an overlay independent of them and let only the position depend on paint. Two tests wanted the contents and could not see them until that split was made. |
...
The test dispatcher runs a **simulated clock**, so anything driven by `timer()` does not advance under `run_until_parked` | An animation stepping through `cx.background_executor().timer(..)` stops after its *first* tick and stays there — real `thread::sleep` does not help either, so `wait_for`'s polling loop times out and the whole feature reads as never having fired. `cx.executor().advance_clock(duration)` is the only thing that moves it. The tab strip's eased reveal cost a debug cycle to this: `tab_scroll_target` was correctly set to `-1520` while the offset sat at `-0.009`, which looks exactly like a clamp eating the write rather than a clock that never ticked. Worth reading with the row below about `flex_none` — both are cases where the state is right and the frame is not. |
